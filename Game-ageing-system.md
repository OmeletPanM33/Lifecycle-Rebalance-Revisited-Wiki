### Citizen ageing
The game has an internal counter for citizen age (not years). The counter updates once per 'simulation step' for that citizen (once per umodded game week).  Previously undefined, the 1.13 game update (Sunset Harbor) introduced a conversion rate of 3.5 age units per year (e.g. a citizen with age 35 is 10 years old) - see 'note to modders' below.
...
After the 1.13 update, citizens now die randomly between 240 and 400(!), depending on how healthy they are (level of healthcare provided); mathematical details are available on the [theory of operation](https://github.com/algernon-A/Lifecycle-Rebalance-Revisited/wiki/Lifecycle-calculations:-theory-of-operation) page.  Prior to 1.13, citizens would all die instantly at 240, which was one factor that create the infamous huge 'deathwaves' that plagued the vanilla game prior to 1.13.  While the 1.13 death age changes have reduced the severity of those deathwaves, the underlying problem is still there (and is addressed by this mod).
